...
> The craft of lexicography demands not only the ability to collect data, but also the ability to make sense of it. -- SUE ATKINS
...
<SideNote type="note">
  ...
  of language through the invention of data is particularly regrettable because
  the focus on boundary cases supported by invented examples left linguistics
  drowning in a welter of imagined possibilities, while the central and typical
  norms of languages as systems were neglected - or were assumed to be so
  obvious that they were not worth studying."
</SideNote>
...
A sign language corpus is a corpus in which the focus is not on the acoustic/articulatory modality of a spoken language, but on the visual/gestural modality of a signed language. Transcription of sign language corpora is typically done with the help of specialized transcription systems for sign languages. The [British Sign Language Corpus](https://bslcorpusproject.org/) is a prototypical example of a sign language corpus. For more information on multimedia corpora (their annotation and encoding) see, for instance, [](https://www.exmaralda.org/files/CLARIN_Standards)Chapter 6 of the [CLARIN Deliverable: D5.C-3 Interoperability and Standards](https://office.clarin.eu/pp/D5C-3.pdf).
...
> **Types and tokens**
>
> The number of **tokens** is the total number of (word) forms in a corpus.
>
> The number of **types** is the number of unique word forms in a corpus.
...
In practice, the ideal corpus composition and size are often hampered by limitations on resources and other practical considerations. The texts that you would ideally like to include in your corpus, may not be available in digital format or they may be subject to some copyright protection or other restrictions on usage.
...
The tags that can be used are defined in a tagset. Different tagsets exist, e.g.

- [Brown tagset](http://korpus.uib.no/icame/manuals/BROWN/INDEX.HTM#bc6)
- [Stuttgart-Tübingen tagset](https://www.ims.uni-stuttgart.de/forschung/ressourcen/lexika/germantagsets/)
...
Semantically annotated corpora are, for instance, used in NLP for Word Sense Disambiguation (WSD).

##### Other types of annotation

Depending on the kind of research other types of annotation can be used or added, e.g. for discourse analysis, in multimedia corpora. For more information on these, we refer to the relevant literature (e.g. Gries and Berez 2017).
...
## Corpora in lexicographic practice

Nowadays corpora have become an integral part of lexicographic practice, but the role of the corpus can differ. Three approaches can be distinguished:

- Corpus-driven

In a corpus-driven project, the corpus data is leading. The entries, sense divisions and definitions are fully consistent with, and reflect directly, the evidence of the corpus (e.g. examples are used verbatim) (Krishnamurthy 2008:240)

- Corpus-based

In corpus-based projects, the corpus plays a major role, but some lexicographical decisions are not based on corpus data, either due to shortcomings of the corpus or due to practical considerations (e.g. all items of a lexical set are included in the dictionary regardless of their occurrence in the corpus) (Kosem 2016)

- Corpus-informed

We speak of corpus-informed when the corpus is used in a limited way, for example, in upgrades of an existing dictionary by adding example sentences from a corpus. (Kosem 2016)

Regardless of the approach taken, corpora are a pivotal source of linguistic information for lexicographic practice. They contain raw and unedited specimens of authentic language. And most importantly, good corpora should be large enough to contain a sufficient number of occurrences of both frequent, less frequent and also rare linguistic phenomena so that information about their use can be extracted or inferred with a high level of reliability.

In this chapter, we will illustrate different kinds of lexicographically relevant information that can be found in corpora, e.g. headwords, collocations, example sentences. This part of the course is closely related to the course LEX2: Mastering ELEXIS Corpus Tools for Lexicographic Purposes, where you will actually learn to use corpus tools. Here we focus on the theory.

### Headword list

Most lexicographic projects will start from a headword list. The headword list defines the size of the lexicographic work because each headword typically becomes a dictionary entry.

A headword list is important both for new lexicographic works created from scratch as well as for updating existing works. With new works, the list defines the size and coverage of the new work which in many cases affects the funding, time frame and workforce required. When updating an existing work, the new headword list should be compared to the existing headwords to identify new or missing lemmas or to identify lemmas which are no longer needed and which can be removed. In the past, in the age of printed dictionaries, entries had to be removed to make space for new entries to keeping the dictionary size manageable. Nowadays, when most works are only published electronically, the need to remove entries hardly exists.

A headword list is generated using a frequency tool such as the wordlist tool in Sketch Engine. As the headword is often the lemmatised form of the word, the list should be generated from a lemmatised corpus so that word form frequencies are counted against the lemma they belong to. The corpus should also be tagged for parts of speech to distinguish between lemmas which can serve as different parts of speech, such as a drink versus to drink, or a party versus to party.

The size of the corpus has to be considered seriously. The frequency of the most frequent lemmas in the language, i.e. the top 1,000 lemmas, can be retrieved reliably from corpora of virtually any size. It is, however, the mid-frequent and especially the rare lemmas where the size of the corpus becomes fundamental. To get reliable information about their frequency, their frequency should be statistically significant. A couple of occurrences cannot provide reliable information because it is impossible to establish whether the few occurrences are just accidental or whether they actually represent the true frequency of the lemma. A large corpus, typically billions of words, is required for rare lemmas to appear at least a few hundred times.

Corpora can also provide information about how words combine.

### N-grams

In general, n-grams are sequences of items. In the context of corpora, n-grams are sequences of tokens (or words). For the purpose of lexicography, the most frequent approach is to generate n-grams containing a headword. Such a result may bring up multiword expressions which might be valuable additions to the dictionary entry. They can also indicate the most frequent syntactic structures.

These are the top twenty 5-grams containing the word end:

![](https://lh6.googleusercontent.com/RgTGQhK7pRNu1pgvpPHqy2dm1de1wpipLlxjCKsWHMLjV7XrYKLOM07VTGsCP29YwlzyvqB5nCgJgBVE_ALLwypqxQg0-SlwMAP91sHQ1IIaAtDBoqeqBHdZ9W-0JvHrwumboB4_NpC2G9lTCQ)

### Collocations

There are several tools which can be used to extract collocation information from the corpus. The choice of the tool depends on the type of collocation.

#### Word Sketch

The Word Sketch is the best starting point for exploring collocations. It identifies words which typically appear together and categorizes them according to the relation between them. Each column represents a different grammatical relation.

![](https://lh6.googleusercontent.com/ZIFA89_yEK-rAkVc3TNJbNNwbGTvyZbszQSyMYr8Co4SlG45_zSgDLjaXP-LvZRmEtHrQrd0KNBldSeqZfYrrigu4ZPqWptqdft4Bv8W2ZAi1LOSQ6Iaw8z_UY1Z_M6XB54HutlxIwwBa0OBqw)

The fact that a word combination is frequent is often of limited use or even insignificant for the purpose of lexicography. For example, these are the most frequent collocations of the word bedroom (only adjectives modifying the noun are included)

_small own spare twin front main comfortable big large_

Looking at the list, one notices that most of the words are very predictable. In other words, if a person wants to speak about a bedroom of a small size, they will naturally use the word small. They will not usually need to consult a dictionary to make sure that small is a suitable word combination. Including such information in a dictionary is not particularly useful.

On the other hand, collocations which are typical, or strong, are far more useful. Typicality often brings up collocations which are not (completely) predictable. _Twin bedroom_ is an example. A collocation list for bedroom ordered by the typicality score will look quite different with these items at the top:

_master double spacious spare en-suite upstairs twin guest air-conditioned_

It contains several collocations which could be very useful if included in a dictionary.

### Example sentences

Example sentences, an integral part of modern dictionaries, can add significant value through the context which they provide for the headword but only if they are selected carefully. As Rundell and Atkins (2008: 454) point out, 'you sometimes find that an entry is almost incomprehensible without its examples.' This argument is strengthened by the recent findings of Frankenberg-Garcia (2012, 2014) that several corpus examples can sometimes be even more useful than the definition.

There are several aspects that should be considered. As summarised in Kosem et al. (2018:120), "the most frequently mentioned characteristics of a good dictionary example are naturalness or authenticity, typicality, informativeness, and intelligibility (see e.g. Fox 1987, Harras 1989, Atkins and Rundell 2008: 458). Naturalness means that the example appears natural, i.e. like the one you would expect to encounter in actual language use. Authenticity of examples is ensured by obtaining them from corpora, which has become a standard practice in modern lexicography. The principle of typicality states that examples must show typical usage of the word in terms of context, syntax, phraseology and the like. An informative example brings added value to the entry, by helping contextualising the use of the word in a particular sense or subsense, thus attesting the information in the definition and helping the user with understanding the definition. Also, an informative sentence should be self-contained, i.e. its content should be understandable without the need for wider context (Harras 1989). Finally, examples that are intelligible (to the users) are those that are not too long and do not contain complex syntax or rare or specialised vocabulary; meeting these criteria is the best way of ensuring that the user's focus remains on the word that is being consulted."

These conditions can be met by only selecting sentences which:

- are not too long to make them difficult to understand but long enough to provide enough context for the headword.
- contain easy to understand, i.e. high-frequency words, but words which constitute a typical context, for example, strong collocations.
- do not contain frequent references pointing outside the sentence, such as a large number of pronouns
- do not require the knowledge of specific extralinguistic reality to be understood.
- avoid controversial topics or topics which make dictionary users feel uneasy because of their cultural or religious background.

This may seem relatively easy, but, as lexicographers know all too well, finding good dictionary examples that meet all of these criteria is a difficult task. For example, short sentences might be preferred over long ones for intelligibility reasons. However, without (enough) context, short example sentences can quickly lose their informative value, and in cases where the original example has been shortened, even their naturalness. In some cases, like in dictionaries for language learners, where the productive value of examples is very important, examples with more context are actually preferred. (Kosem et al. 2018: 120)

### Neologisms

Corpora can also be used to find new words. Neologisms are of great interest to lexicographers because they are the first to be judged by the dictionary users when determining how up-to-date the lexicographic product is. Metcalf (2002) suggested 5 factors (referred to as FUDGE scale) that help us determine the probability of a word's survival: (1) frequency of the word; (2) unobtrusiveness, i.e., a successful word should not be exotic or too cleverly coined; (3) diversity of use, i.e., the range users and contexts; (4)

generation of other forms and meanings, namely the productivity of the word; and (5) endurance of the concept.

A traditional approach involved human researchers reading or scanning large amounts of text such as newly published books, newspapers or magazines, in the hope that they come across a new word. This procedure was extremely time-consuming and labour-intensive and produced only a handful of new words in a non-systematic way. The situation called for an automatic procedure for detecting neologisms.

An automatic detection of neologisms is, however, extremely complicated. The main problem is that all neologisms first appear as very infrequent lexical items and their frequency rises in time with variable intensity. A lexical item is only considered a neologism while it is new, not well established in the language yet, and therefore, while their frequency is still relatively small. Once their frequency becomes high and they are used widely, they are usually no longer considered neologisms but established vocabulary. Unfortunately, it is not viable to measure how established a lexical item is. The automatic mechanism is therefore left with the change of the frequency of use in time.

For this to work, time-stamped data are required. Time-stamped refers to data which come with time stamps, such as the date of publication or the date of release.

### Word senses

Corpora can also help to identify the different senses of a polysemous word. For instance, by looking at the concordances (i.e. …) of the noun party in the example below, we see occurrences of the different senses of the noun party, i.e. party in the sense of political party, but also in the sense of a social event. ![](https://lh3.googleusercontent.com/rJ1rFQV9guLxdP0QQV9orXMhyB3Ocw8lyfR-RrcQOgtHRziYhXNuAtA8sB2m_5duArn2imyXr53hrne1gM0zePZBFtNLtRh0NXZU_YWbJoJXYjeTvZ5FsQAaUPX29Eed8HRuf7_AF5u2Ca0q8A)

These different senses can also be observed when looking at the word sketch of party:

![](https://lh5.googleusercontent.com/1YKprossoH1o4bz5qwO9r3PaTmyE16UKYyyYtzOxJMGM8AgQQj6Xqxx50lBh1hhhtk1KhBdCfVoDYDMhc-26EfpXG6k5bzQHsYlEiGtsGhIFoG-0EpQQELGr2hqaPCvd31a8CZq4aWU-eYzNug)

### Labels

Dictionary labels give information about the restricted use of a word or its sense. A label may alert the dictionary user to the fact that a word is mainly use in medical context or when talking about finance. Analysing the metadata in the corpus helps us discover such restricted usages and come up with suitable labels. For example, a corpus containing British English and American English may contain words or word senses which are mainly used in British English. This information can only be retrieved from the corpus, provided the documents in the corpus are classified according to the variety of English.

This screen short shows that word outback is a lot more frequent (has a higher relative density) in texts collected from the Australian .au top-level domain. The relative density is not affected by the difference in the volume of text collected from each domain.

![](https://lh5.googleusercontent.com/It4uGokPFMWHytNPEMD-HAL01L8oUcnSX1_IXKW_LiY_hDrGXHfok1TNEGRwMBD0AvKuOUiTpgLtNNhdBCBK48odb66GOONJ55YkoT5MfzVx-BKJBFaX14sGxrG7A9IcVw-O4C8vzYRPjL0ndg)

Similarly, a corpus with metadata classifying documents into topics can provide information about the usage restrictions of collocations identified by the word sketch:

![](https://lh4.googleusercontent.com/cii22e4QwQLbyGoyAILZxRgCS07z6XSeoV6GlMPV07O-MGquVgZl0FDSYJCe2mm6q1flA3v_es4BAGLbGGxT5cZQGnDhbhoyrxGE9qcwttVpBQxNdGiVgFSaDt5wRqGU4TGP5iOGTmyYU9lpcA)

### Translations

Parallel corpora can be useful for providing translation equivalents. An example is the EUROPARL corpus containing transcripts of speeches given in the European Parliament which were translated into all official languages of the European Union.
...
![](https://lh5.googleusercontent.com/uwiK5N7EU0Eyb3rYCL5klLO9YuqHmNbWtzlaHkIzBUrSFdgsELoGpx792ldkgt76MWVKds3fodls8xXBtVP-pJpSDDWLrcKD3lyS5QYZcLadiP646ZW0EOBmx9eTVUpkHZgtEFCQtOty5OfwvQ)

### Syntactic patterns

There are several ways to explore syntactic patterns of a word.

#### N-grams for discovering patterns

Generating n-grams of different sizes that contain the word can show prepositions and other function words frequently found before and after the word. In Sketch Engine, the ADVANCED tab of the n-gram tools lets the user specify various search options and restricting conditions.

![](https://lh6.googleusercontent.com/EBoY2x5ncS2SlOurDIigmizTIJ4ekpCdTBPuO1Zo8QiqVmA8u5XHv6ndJeb4joeWaFXJzKzzYSX6OFRBFozSZpHiE_7UOmIH1veF1MBig1lUyaNogoURd1SKAxSA59-Fb_eZ9410dD_xcB1L5w)
...
![](https://lh4.googleusercontent.com/sE77wTNcEmMGBk4UbN7xkMlhZNQ7hC8NOzC-r9C_FAdelZFyx1_gN4PXXOlEYV-iB6cWfXwlrp7fMizC25F1H5RrlmThXoKTO_R7IFHH0gmJjD-O-6Xn-UPqvP8De-c2HdwN_AgBrnZi42FQpA)
...
Frequency distribution refers to a function that operates like a calculator of concordance lines. It produces counts of words, lemmas, tags and other attributes in specific positions of a concordance line. This frequency distribution shows the frequency of the POS attribute following a/the total of. It is easy to see that a total of has a strong tendency to be followed by a number (e.g. a total of 15 people) while the total of is frequently followed by noun. A close look a the actual lines would reveal that other (x) and adjective (j) refers to a determiner followed by a noun or adjective followed by a noun.

![](https://lh5.googleusercontent.com/KS9KJFrEXcVEqOwVtIAmp6gffBzUUM17C5PA2z2GjCTi-wD-Fe-sN_hzyImB0BQ7ckk8yFo0N6tznwnxs4Oxe-4n6xyHvOYRDx_MCORtm_OIaR1sYq0KM8yVnySu9BYIeH5lWO5raAL6kLyGvg)

A similar technique can be used to analyse the left or right context or of other lexical or grammatical structures.

### Statistics

Different tools which are part of a corpus software offer various types of statistics. It is beyond the scope of this course to provide a detailed explanation of each of them. However, when using Sketch Engine, the interface provides quick reference as well as detailed information about the various types of statistics via tooltips that are displayed upon hovering the mouse over the statistic or over the column header. The tooltips frequently contain links to more detailed information.

![](https://lh5.googleusercontent.com/tP9qIUGtSQFZNTWUO_7vFuONBqNNrYtjq4EQdpDy7cWPVE7iuL2rke0PYvNp7co0nRa6I6aB-X0fJxRKAIOB86rLTWpTu4HoEb_AUQ2WUOonU-XgXed06zTMxOxNeqlPumbnKiCBPZLPLbDEYQ)

#### Corpus size

The size of the corpus is typically given in the number of words. Different corpus systems, however, have different ways of defining what a word is and it is common to see slightly different sizes depending on which corpus software is used. Sketch Engine provides different size measures, the most frequently used ones are the size in words and tokens whose definitions can be found on these links:

[https://www.sketchengine.eu/my_keywords/token/](https://www.sketchengine.eu/my_keywords/token/)

[https://www.sketchengine.eu/my_keywords/word/](https://www.sketchengine.eu/my_keywords/word/)

#### Frequency

The frequency is the basic statistics in corpora. It refers to the number of hits (the number of times the search word or phrase was found). It can only be used to compare information derived from the same corpus. It cannot be used to compare two different corpora because two corpora have different corpus sizes and, naturally, a larger corpus will produce a higher frequency than the smaller corpus. See also [https://www.sketchengine.eu/my_keywords/frequency/](https://www.sketchengine.eu/my_keywords/frequency/)

#### Relative frequency, normalized frequency, frequency per million

All three terms refer to the same statistic. Relative frequency is used to compare information derived from corpora of different sizes. The value does not depend on the corpus size. Sketch Engine uses frequency per million. Thus, a relative  frequency of 10 per million means that there are 10 hits in each million tokens on average. If the corpus is smaller than a million, it indicates what the frequency would be if the corpus had a million tokens. See also [https://www.sketchengine.eu/my_keywords/freqmill/](https://www.sketchengine.eu/my_keywords/freqmill/)

#### Association scores and collocation metrics

There are several statistical methods to discover collocations and to measure how strong, important or typical the relationship between the lexical items is.

Historically, it was the T-score and MI Score that were used most frequently but the current corpora, which are many times larger than the corpora 30 years ago, require a more suitable metric such as the logDice.

##### T-score

The T-score expresses the certainty with which we can argue that there is an association between the words, i.e. their co-occurrence is not random. It will bring to the top the co-occurrences which are frequent enough.

Unfortunately, with multi-billion-word corpora, the corpus size affects the score to such an extent, that it mainly brings up co-occurrences which are only frequent without being particularly interesting or useful.

See also [https://www.sketchengine.eu/my_keywords/t-score/](https://www.sketchengine.eu/my_keywords/t-score/)

##### MI Score

The Mutual Information Score will highlight pairs of words that frequently appear together and rarely combine with other words. It will successfully detect fixed phrases or technical terms.

Since the score is affected by the corpus size, it is not particularly useful with modern large corpora where it frequently detects various random fixed items of little interest, for example, combinations of words which are misspelt and only appear very infrequently in the corpus.

See also [https://www.sketchengine.eu/my_keywords/mi-score/](https://www.sketchengine.eu/my_keywords/mi-score/)

##### logDice

The logDice score is a better option suited to large corpora. It is only based on the frequency of the node and the collocate and the frequency of the whole collocation (co-occurrence of the node and collocate). logDice is not affected by the size of the corpus and, therefore, can be used to compare scores between different corpora.

See also [https://www.sketchengine.eu/my_keywords/logdice/](https://www.sketchengine.eu/my_keywords/logdice/)

#### Keyness score

The keyness score refers to any type of score used to identify keywords which are words with an unusually high frequency compared to some reference data, typically compared to another corpus. Keywords can also be understood as lexical items which are typical of or representative of your data when making a comparison with the reference data.

##### Simple Math

The score used in Sketch Engine is called Simple Maths. It is used to identify keywords (single word items), terms (mutliword items), key n-grams and also key collocations. See also [https://www.sketchengine.eu/my_keywords/simple-maths/](https://www.sketchengine.eu/my_keywords/simple-maths/)
...
Baker, M. (1995). Corpora in Translation Studies. An Overview and Some Suggestions for Future Research. Target, 7. 223–243. [https://doi.org/10.1075/target.7.2.03bak](https://doi.org/10.1075/target.7.2.03bak)
...
Gries, S. and· A.L. Berez (2017). [Linguistic Annotation in/for Corpus Linguistics](https://scholarspace.manoa.hawaii.edu/server/api/core/bitstreams/2a1388a5-104f-4495-b547-068b7ec04ccf/content). In N. Ide and J. Pustejovsky (eds.), Handbook of Linguistic Annotation. 379-409.
...
Kosem, I., K. Koppel, T. Zingano Kuhn, J. Michelfeit and C. Tiberius (2018). Identification and automatic extraction of good dictionary examples: the case(s) of GDEX. In International Journal of Lexicography, Volume 32, Issue 2, June 2019. 119–137, [https://doi.org/10.1093/ijl/ecy014](https://doi.org/10.1093/ijl/ecy014).
